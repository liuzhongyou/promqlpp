...
class Time {
  ...
  Time(std::uint64_t wall, std::int64_t ext)
      : wall{wall}
      , ext{ext} {
  }
  ...
  // TODO(mirco): support nanosecond precision when parsing date
  ...
  std::string String() {
    ...
    // TODO(mirco): truncate trailing 0s to guarantee parity with Go
